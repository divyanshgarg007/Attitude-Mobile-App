...
const CategorySaleItemsView = (props) => {
    ...
    const callApi = () => {
        let { actions } = props;
        let params = "sale=Uitverkoop"
        actions.getCategoryProduct({ id: `${item._attrs.id}&start=0&count=${PAGE_SIZE}&${params}`, filter: `sort=isort1|desc` }, onSuccess, onError);
    }
    ...
    const getFiter = (tag) => {
        ...
        let { actions } = props;
        let data = "sale=Uitverkoop"
        actions.getCategoryProduct({ id: `${props.route.params.data._attrs.id}&start=0&count=${PAGE_SIZE}&${data}`, filter: params }, onFilterSuccess, onError)
    }

    const onClickItem = (item) => {
        console.log(item)
        props.navigation.push('ArticleDetail', { data: item, type: 'sale' })
    }

    const onClickFilterItem = (obj) => {
        props.navigation.push('ArticleSaleView', { data: obj })
    }

    const onClickSortItem = (index) => {
        setSelectedIndex(index)
        // isort1|desc                    Popular, use with |desc to get the most popular first
        // isort1                  Date added (use for “Newest items”)
        // nunitprice           Price
        ...
        let { actions } = props;
        let params = "sale=Uitverkoop"
        actions.getCategoryProduct({ id: `${props.route.params.data._attrs.id}&start=0&count=${PAGE_SIZE}&${params}`, filter: filterParam }, onSuccess, onError);
    ...
    const loadMoreData = () => {
        if (!request.loading && request.morePage) {
            ...
            let { actions } = props;
            let params = "sale=Uitverkoop"
            actions.getCategoryProduct({ id: `${props.route.params.data._attrs.id}&start=${request.start}&count=${PAGE_SIZE}&${params}`, filter: filterParam }, onSuccess, onError);
        }
    }
    ...
    return (
        <SafeAreaView style={{ flex: 1, backgroundColor: "#fff" }}>
            ...
            />
            <FilterView data={item.webgroup} onClick={(obj) => onClickFilterItem(obj)} type='sale' />
            <ArticleGrid
                type='sale'
...
export default connect(mapStateToProps, mapDispatchToProps)(CategorySaleItemsView)
