...
const MyOrdersView = (props) => {
    ...
    return (
        <SafeAreaView style={styles.container}>
            {/* <NavigationBar title="MY ORDERS" navigation={props.navigation} /> */}
            <FlatList
                data={DATA}
...
const Header = () => (
    <View style={styles.headerContainer} >
        <Text style={styles.textHi}>25 orders</Text>
    </View>
)
    ...
    <Pressable style={{ paddingHorizontal: 15 }} onPress={() => props.onClickItem(props.item)} >
        <View style={styles.item}>
            <View>
                ...
                <Text style={styles.textId}># {props.item.orderId} - {props.item.date}</Text>
            </View>
...
const ProductItem = (props) => {
    ...
    for (let index = 0; index < props.items.length; index++) {
        if (index <= 1) {
            arr.push(
                <View key={index} style={styles.prodItem} />
            )
        } else {
